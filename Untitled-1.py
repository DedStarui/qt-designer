...
class Widget(QMainWindow):
    def __init__(self):
        ...
        self.ui.pushButton_7.clicked.connect(self.add_tag)
        self.ui.pushButton.clicked.connect(self.delete_tag)
        self.ui.pushButton.clicked.connect(self.search_by_tag)
    ...
    def delete_tag(self):
        if self.ui.listWidget_2.selectedItems():
            ...
            self.ui.listWidget_2.clear()
            self.ui.lineWidget_2.addItems(self.notes[key]["теги"])
            with open("notes_data.json", "w", encoding = "utf-8") as file:
                json.dump(self.notes,file,sort_keys=True)
        else:
    ...
    def search_by_tag(self):
        ...
        if ok and tag!="":
            self.ui.listWidget.clear()
            self.ui.textEdit.clear()
            matching_notes = [note for note, data in self.note.items() if tag in data["теги"]]
